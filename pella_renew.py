...
# 请根据 Pella 实际的控制台地址进行调整
LOGIN_URL      = "https://www.pella.app/"
FREE_PANEL_URL = "https://www.pella.app/free_panel" 
    ...
    with SB(uc=True, test=True, proxy=LOCAL_PROXY) as sb:
        ...
        for _ in range(20):
            ...
            if turnstile_exists(sb):
                print("🛡️ 检测到Turnstile...")
                if not solve_turnstile(sb):
                    sb.save_screenshot("pella_cf_fail.png")
        ...
        if not clicked:
            print("❌ 继续按钮缺失")
            sb.save_screenshot("pella_no_continue_btn.png")
            send_tg("❌ 继续按钮缺失")
            return
        ...
        try:
            ...
            print("❌ OTP框加载失败")
            sb.save_screenshot("pella_no_otp.png")
            send_tg("❌ OTP框加载失败")
            return
        ...
        try:
            ...
            print(e)
            sb.save_screenshot("pella_otp_timeout.png")
        ...
        if not verify_clicked:
            print("❌ 验证按钮缺失")
            sb.save_screenshot("pella_no_verify_btn.png")
            send_tg("❌ 验证按钮缺失")
            return
        ...
        for _ in range(80):
            ...
            print("❌ 登录等待超时")
            sb.save_screenshot("pella_login_timeout.png")
            send_tg("❌ 登录等待超时")
            return
